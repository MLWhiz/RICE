...
class SolitaireMancala:
    ...
    def set_board(self, configuration):
        ...
        self.start_config = self.board
        
    def __str__(self):
        """
        Return string representation for Mancala board
        """
        s = ", ".join([str(x) for x in self.board[::-1]])
        return "["+s+"]"
    ...
    def plan_moves(self):
        ...
        """
        self.moves = []
        while self.is_game_won()==False and self.choose_move()!=0:
            self.moves.append(self.choose_move())
            self.apply_move(self.choose_move())

        self.set_board(self.start_config)
        return self.moves
